...
const resolver = {
    ...
    login: async ({id, pw}, req) => {
        if(req.user) console.log(`user = ${JSON.stringify(req.user)}`)//delete it
    ...
    comments: async({postID}) => {
        ...
        else return JSON.stringify(comments, undefined, 2)
    },

    geoJSON: async(_) => {
        await Dataset.aggregate(conditions, (err, data) => {
            if(!err) {
                let geodata = GeoJSON.parse(data, {Point: ['latitude', 'longitude']});
                _data = JSON.stringify(geodata, undefined, 2)
            }
        })
        return _data
    }
